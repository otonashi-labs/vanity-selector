...
#define BRUTFORCE_STRING_SIZE PYTHON_INJECTION_5
#define KERNEL_ITERATION_SEED PYTHON_INJECTION_6

__constant char charset[62] = "0123456789ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz";

__constant char target_prefix[9] = TARGET_PREFIX;
__constant char prefix[] = STRING_PREFIX;
__constant char suffix[] = STRING_SUFFIX;

#define LCG_MULTIPLIER 6364136223846793005UL
#define LCG_INCREMENT 1UL

ulong lcg_rand(ulong *seed) {  
    *seed = (*seed * KERNEL_ITERATION_SEED * LCG_MULTIPLIER + LCG_INCREMENT);  
    return *seed;  
}  

ulong rand_mod(ulong *seed, ulong mod) {  
    return lcg_rand(seed) % mod;  
}  
...
__kernel void keccak_hash(__global uchar *output_flags) {
    size_t gid = get_global_id(0);
    ulong seed = gid;

    // Generate random string of length BRUTFORCE_STRING_SIZE
    char random_string[BRUTFORCE_STRING_SIZE + 1]; 

    for (int i = 0; i < BRUTFORCE_STRING_SIZE; i++) {  
        ulong char_index = rand_mod(&seed, (ulong)62u);  
        random_string[i] = charset[char_index];  
    }  
    random_string[BRUTFORCE_STRING_SIZE] = '\0'; 
    ...
    int total_len = prefix_len + BRUTFORCE_STRING_SIZE + suffix_len;

    // Ensure the message buffer is large enough
    uchar msg[300] = {0}; // Adjust size if needed
    ...
    for (int i = 0; i < BRUTFORCE_STRING_SIZE; i++, pos++) {
        msg[pos] = random_string[i];
    }
